...
export class DisciplinaPage implements OnInit {
  ...
  constructor(private disciplinaService: DisciplinaService, private activatedRoute: ActivatedRoute,private router: Router,
    private toastCtrl: ToastController) { }

  ngOnInit() {
    this.codigo= this.activatedRoute.snapshot.paramMap.get('codigo');
    if(this.codigo){
      this.disciplinaService.getDisciplinaPorCodigo(this.codigo).subscribe(
          (result)=>{
